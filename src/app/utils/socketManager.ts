...
// Define the type for the customer socket ID mapping
type CustomersInRooms = { [roomId: string]: string[] }

// Keep track of customer socket IDs in each room
const customersInRooms: CustomersInRooms = {}

// Define the type for the active users mapping
type ActiveUsers = Map<string, string>

// Keep track of active users
const activeUsers: ActiveUsers = new Map()
  ...
  io.on('connection', (socket: Socket) => {
    ...
    socket.on('joinRoom', (roomId: string) => {
      ...
      const roomAgents = Array.from(activeUsers.keys()).filter(userId => {
        ...
        return (
          agentSocketId && !customersInRooms[roomId]?.includes(agentSocketId)
        )
      })
    ...
    socket.on('customerMessage', (message: string, roomId: string) => {
    ...
    socket.on(
      'agentMessage',
      (message: string, roomId: string, customerId: string) => {
        // Find the socket ID of the selected customer in the room
        const customerSocketId = customersInRooms[roomId]?.find(
          id => id === customerId
        )
