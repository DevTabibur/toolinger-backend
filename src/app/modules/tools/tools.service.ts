...
import { ProxyAgent, ProxyAgentOptions } from "proxy-agent"; // ✅ Ekhanei poriborton
...
const BacklinkMaker = {
  generateBacklink: (input: BacklinkInput): string => {
...
export const PingService = {
  sendPings: async (input: PingInput) => {
...
const KeywordDensity = async (url: string) => {
...
const checkMalwareWithGoogle = async (url: string) => {
  ...
  const reportUrl = `https://transparencyreport.google.com/safe-browsing/search?url=${encodeURIComponent(
    url
  )}&hl=en`;
...
const DomainToIP = async (domain: string) => {
  try {
    const cleanDomain = extractHostname(domain);
    const { address } = await dns.lookup(cleanDomain);
    ...
    throw new Error(
      "Failed to fetch domain info. Please check the domain name."
    );
  }
};
...
const ServerStatusChecker = async (
  urlsInput: string
): Promise<SiteStatus[]> => {
  ...
  for (const url of urls) {
    ...
    try {
      ...
      results.push({
        url,
        httpCode: err?.response?.status || null,
        responseTime: null,
        status: "Offline",
      });
    }
...
const PageSizeChecker = async (url: string) => {
  try {
    const response = await axios.get(url, {
...
const BlacklistLookup = async (domain: string) => {
  try {
...
const checkSuspiciousDomains = async (urlList: string[]) => {
  if (urlList.length === 0 || urlList.length > 20) {
    throw new Error("You must provide 1 to 20 URLs.");
  }

  const results = await Promise.all(
    urlList.map((url) => checkIfDomainIsSuspicious(url.trim()))
  );
...
const CodeToCheckTextRatio = async (input: { urls?: string[], url?: string }) => {
...
const LinkCountChecker = async (url: string) => {
  try {
    const response = await axios.get(url);
    const html = response.data;
    const { total, internal, external } = parseLinks(html, url);
...
const EmailPrivacycheck = async (url: string) => {
  try {
    const response = await axios.get(url);
    const html = response.data;
...
const MetaTagAnalyze = async (url: string) => {
  try {
    // const response = await axios.get(url);
    // const html = response.data;
...
const searchEngineSpiderSimulator = async (url: string) => {
...
const checkGoogleCache = async (url: string) => {
...
const getBrowserInfo = (req: Request) => {
...
const findDNSRecords = async (domain: string) => {
...
const getClassCInfo = async (domains: string[]) => {
  const results = await Promise.all(
...
const fetchPageSource = async (url: string): Promise<string> => {
  try {
    const response = await axios.get(url, {
      headers: {
        "User-Agent": "Mozilla/5.0",
...
const rewriteUrl = (inputUrl: string) => {
  try {
...
type BotRule = {
  bot: string;
  rule: "Allow" | "Disallow" | "Same as Default";
};
...
const generateRobotsTxt = (options: RobotsOptions): string => {
  const lines: string[] = [];
...
const generateSitemapXML = (urls: SitemapUrl[]): string => {
  ...
  urls.forEach((url) => {
    ...
    xmlParts.push(`<priority>${url.priority ?? 0.1}</priority>`);
    xmlParts.push(`<changefreq>${url.changefreq ?? "always"}</changefreq>`);
    xmlParts.push(`<lastmod>${url.lastmod ?? now}</lastmod>`);
    xmlParts.push(`</url>`);
  });
