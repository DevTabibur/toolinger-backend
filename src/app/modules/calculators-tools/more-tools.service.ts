...
const convertRomanNumeralsDate = async (data: any) => {
  ...
  try {
    ...
    throw new ApiError(
      httpStatus.BAD_REQUEST,
      "Failed to convert Roman numerals date",
    );
  }
};
...
const translateBinary = async (data: any) => {
  ...
  if (!input || !inputType || !outputType) {
    throw new ApiError(
      httpStatus.BAD_REQUEST,
      "Input, inputType, and outputType are required",
    );
  }
  ...
  try {
    ...
    throw new ApiError(
      httpStatus.BAD_REQUEST,
      "Failed to translate value: " + error.message,
    );
  }
};
...
const generateRandomAddress = async (data: any) => {
  ...
  if (!countryData[country]) {
    throw new ApiError(
      httpStatus.BAD_REQUEST,
      "Country not supported. Supported countries: " +
  ...
  try {
    ...
    throw new ApiError(
      httpStatus.BAD_REQUEST,
      "Failed to generate random address",
    );
  }
};
...
const calculateDiscount = async (data: any) => {
  ...
  if (!originalPrice || discountPercentage === undefined) {
    throw new ApiError(
      httpStatus.BAD_REQUEST,
      "Original price and discount percentage are required",
    );
  }
...
const convertBinaryToHex = async (data: any) => {
  ...
  try {
    ...
    throw new ApiError(
      httpStatus.BAD_REQUEST,
      "Failed to convert binary to hex",
    );
  }
};
...
const convertDecimalToOctal = async (data: any) => {
  ...
  try {
    ...
    throw new ApiError(
      httpStatus.BAD_REQUEST,
      "Failed to convert decimal to octal",
    );
  }
};
...
const convertOctalToDecimal = async (data: any) => {
  ...
  try {
    ...
    throw new ApiError(
      httpStatus.BAD_REQUEST,
      "Failed to convert octal to decimal",
    );
  }
};
...
const calculateOctal = async (data: any) => {
  ...
  if (num1 === undefined || num2 === undefined || !operation) {
    throw new ApiError(
      httpStatus.BAD_REQUEST,
      "Two numbers and operation are required",
    );
  }
...
const convertDecimalToASCII = async (data: any) => {
  ...
  try {
    ...
    throw new ApiError(
      httpStatus.BAD_REQUEST,
      "Failed to convert decimal to ASCII",
    );
  }
};
...
const calculateAdsense = async (data: any) => {
  ...
  if (!pageViews || !cpm || clickRate === undefined) {
    throw new ApiError(
      httpStatus.BAD_REQUEST,
      "Page views, CPM, and click rate are required",
    );
  }
...
const calculatePaypalFee = async (data: any) => {
  ...
  try {
    ...
    throw new ApiError(
      httpStatus.BAD_REQUEST,
      "Failed to calculate PayPal fee",
    );
  }
};
...
const generateUpsideDownText = async (data: any) => {
  ...
  try {
    ...
    const upsideDown = text
      ...
      .reverse()
      .map((char: string) =>
        upsideDownMap[char] !== undefined ? upsideDownMap[char] : char,
      )
      .join("");
    ...
    throw new ApiError(
      httpStatus.BAD_REQUEST,
      "Failed to generate upside down text",
    );
  }
};
...
const generatePassword = async (data: any) => {
  ...
  if (!length || typeof length !== "number" || length < 1 || length > 64) {
    throw new ApiError(
      httpStatus.BAD_REQUEST,
      "Password length must be between 1 and 64",
    );
  }
...
const convertRomanNumeral = async (data: any) => {
  ...
  if (data.decimal !== undefined && data.roman !== undefined) {
    throw new ApiError(
      httpStatus.BAD_REQUEST,
      "Provide only one of 'decimal' or 'roman'",
    );
  }
  ...
  if (data.decimal !== undefined) {
    ...
    if (isNaN(decimal) || decimal <= 0 || decimal >= 4000) {
      throw new ApiError(
        httpStatus.BAD_REQUEST,
        "Decimal must be a number between 1 and 3999",
      );
    }
    ...
    if (!output) {
      throw new ApiError(
        httpStatus.BAD_REQUEST,
        "Invalid decimal number for Roman numeral conversion",
      );
    }
    ...
    if (isNaN(decimal) || decimal <= 0 || decimal >= 4000) {
      throw new ApiError(
        httpStatus.BAD_REQUEST,
        "Invalid Roman numeral for decimal conversion",
      );
    }
    ...
    throw new ApiError(
      httpStatus.BAD_REQUEST,
      "Provide either 'decimal' or 'roman'",
    );
  }
...
const calculateCPM = async (data: {
  ...
  if (!["cost", "impressions", "cpm"].includes(calculate)) {
    throw new ApiError(
      httpStatus.BAD_REQUEST,
      "Invalid 'calculate' value. Must be 'cost', 'impressions', or 'cpm'.",
    );
  }
  ...
  if (calculate === "cost") {
    if (
      ...
      throw new ApiError(
        httpStatus.BAD_REQUEST,
        "Invalid input: impressions must be > 0 and cpm must be >= 0.",
      );
    }
    ...
    if (
      ...
      throw new ApiError(
        httpStatus.BAD_REQUEST,
        "Invalid input: cost must be >= 0 and cpm must be > 0.",
      );
    }
    ...
    if (
      ...
      throw new ApiError(
        httpStatus.BAD_REQUEST,
        "Invalid input: cost must be >= 0 and impressions must be > 0.",
      );
    }
  ...
  return {
    ...
    impressions:
      typeof impressions === "number" ? Math.round(impressions) : undefined,
...
const generateRandomWord = (payload: {
  ...
  if (
    ...
    throw new ApiError(
      httpStatus.BAD_REQUEST,
      "Please provide a valid number of generations.",
    );
  }
  ...
  if (pool.length === 0) {
    throw new ApiError(
      httpStatus.BAD_REQUEST,
      "No words available for the selected type.",
    );
  }
...
const calculateEPS = (payload: {
  ...
  if (
    ...
    throw new ApiError(
      httpStatus.BAD_REQUEST,
      "Please provide valid netIncome and numberOfShares (numberOfShares must be greater than 0).",
    );
  }
...
const calculateSalesTax = (payload: {
  ...
  if (
    ...
    throw new ApiError(
      httpStatus.BAD_REQUEST,
      "Please provide valid salesTaxPercent (>= 0) and netPrice (>= 0).",
    );
  }

  const salesTaxAmount = Number(
    ((salesTaxPercent / 100) * netPrice).toFixed(2),
  );
...
const calculateAverage = (payload: { values: number[] }) => {
  ...
  if (
    ...
    throw new ApiError(
      httpStatus.BAD_REQUEST,
      "Please provide an array of valid numbers in 'values'.",
    );
  }
...
const wordsToPages = (payload: {
  ...
  if (
    ...
    throw new ApiError(
      httpStatus.BAD_REQUEST,
      "Invalid fontFamily, fontSize, or spacing value.",
    );
  }
  ...
  if (typeof words !== "number" || isNaN(words) || words < 0) {
    if (typeof text === "string") {
      ...
      throw new ApiError(
        httpStatus.BAD_REQUEST,
        "Please provide a valid 'words' number or a 'text' string.",
      );
    }
  }
...
const calculateValuation = (payload: {
  ...
  if (provided.length < 2) {
    throw new ApiError(
      httpStatus.BAD_REQUEST,
      "Please provide at least two of: preMoney, postMoney, investment.",
    );
  }
  ...
  if (
    ...
    throw new ApiError(
      httpStatus.BAD_REQUEST,
      "Invalid input values for valuation calculation.",
    );
  }
 ...
 * }
 */
const calculateBounds = async (payload: {
  ...
  if (
    ...
    throw new ApiError(
      httpStatus.BAD_REQUEST,
      "Please provide valid values for mean, stdDev, sampleSize (>=2), and confidenceLevel.",
    );
  }
 ...
 */
const calculateMargin = async (payload: {
  cost: number;
  sellingPrice: number;
}) => {
  ...
  if (
    ...
    throw new ApiError(
      httpStatus.BAD_REQUEST,
      "Please provide valid values for cost and sellingPrice (both >= 0).",
    );
  }

  const margin = sellingPrice - cost;
  const marginPercentage =
    sellingPrice === 0 ? 0 : (margin / sellingPrice) * 100;
...
const calculateGST = async (payload: { amount: number; gstRate: number }) => {
  ...
  if (typeof amount !== "number" || typeof gstRate !== "number") {
    throw new ApiError(
      httpStatus.BAD_REQUEST,
      "Amount and GST rate must be numbers.",
    );
  }
  if (amount < 0 || gstRate < 0) {
    throw new ApiError(
      httpStatus.BAD_REQUEST,
      "Amount and GST rate must be non-negative.",
    );
  }
 ...
 * }
 */
const calculateProbability = async (payload: {
  total: number;
  eventA: number;
  eventB?: number;
}) => {
